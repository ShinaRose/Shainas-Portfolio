...
export const featuredWork = [
  ...
  },
  {
    ...
    approach: [
      "Built a Python (pandas) pipeline that scores every supplier on OTIF, lead-time mean and variance against schedule, defect rate and price variance from the real DataCo Smart Supply Chain dataset (Kaggle, 180,519 order lines) — not a mocked-up sample.",
      "Since the dataset has no supplier ID, QC field or contract price table, defined and documented explicit proxies for each rather than pretending the data was cleaner than it is.",
    ...
    outcome: "A working, reproducible scorecard that ranked all 11 supplier groups and correctly surfaced that late delivery in this dataset is driven far more by shipping mode (0%–60% on-time) than by any individual supplier (37%–41.5%) — the kind of root-cause distinction that changes what a procurement team should actually act on.",
